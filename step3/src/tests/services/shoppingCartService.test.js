...
describe('Shopping Cart Service', () => {
    ...
    test('Gets the sales tax for a quantity of items', () => {
        var shoppingCart = new shoppingCartService();
        var item = new product("Toothpaste", 10.99);

        shoppingCart.addToCart(item, 1);

        var taxes = shoppingCart.getSalesTax();

        expect(taxes).toBe(1.37);
    });

    
    test('Gets the sales tax for a quantity of different items', () => {
        var shoppingCart = new shoppingCartService();
        var item1 = new product("Dove Soap", 39.99);
        var item2 = new product("Axe Deo", 99.99);
        ...
        expect(taxes).toBe(35.00);
    });

    test('Get total with taxes returns cart total with taxes for one item', () => {
        var shoppingCart = new shoppingCartService();
        var item = new product("Toothpaste", 10.99);

        shoppingCart.addToCart(item, 1);

        var totalWithTax = shoppingCart.getTotalWithTax();

        expect(totalWithTax).toBe(12.36);
    });

    test('Gets the total with taxes with different items', () => {
        var shoppingCart = new shoppingCartService();
        var item1 = new product("Dove Soap", 39.99);
        var item2 = new product("Axe Deo", 99.99);
        ...
        expect(totalWithTax).toBe(314.96);
    });
});